...
# IMPORTANT:
# This is the ONLY oi_levels() definition.
# ============================================================
...
def oi_levels(chain, spot):
    ...
    # Do not allow a distant strike to become a misleading
...
def score_option(
    ...
    not a statistical probability of winning.
    """
...
def build_plan(
    ...
    if side == "CE":
        ...
        trigger = (
            f"Enter only after spot breaks "
            f"and sustains above "
            f"{fmt_price(trigger_level)}."
        )

        exit_rule = (
            f"Exit if spot loses support "
            f"{fmt_price(support)} or premium hits "
            f"{fmt_price(sl)}. After Target 1, "
            f"book partial profit and trail."
        )
    ...
    # --------------------------------------------------------
        ...
        trigger = (
            f"Enter only after spot breaks "
            f"and sustains below "
            f"{fmt_price(trigger_level)}."
        )

        exit_rule = (
            f"Exit if spot reclaims resistance "
            f"{fmt_price(resistance)} or premium hits "
            f"{fmt_price(sl)}. After Target 1, "
            f"book partial profit and trail."
        )
...
def get_fno_underlyings():
    ...
    for item in records:
        ...
        # Upstox BOD JSON normally supplies expiry as
        # epoch-millisecond value for NSE_FO.
        # Handle both epoch and YYYY-MM-DD formats.
        ...
        try:
            ...
            for raw in rows:
                ...
                for side, action in (
                    ...
                    # One highest-PoP opportunity
                    # per stock keeps Top 5 diversified.
...
try:
    ...
    with st.spinner(
        ...
        # Keep existing UI terminology
        # while using stronger daily trend
...
# Evaluate slightly wider ATM band.
# Choose strongest quality score rather
# than simply highest PoP.
...
with tab1:
    ...
    if decision == "NO TRADE":
        ...
        st.info(
            ...
            "meet the directional quality gate. Wait for the "
            "entry trigger instead of forcing an option position."
        )
    ...
    reasons = [
        ...
        f"Overall direction: {overall_direction} "
        f"with {overall_alignment}/3 timeframes aligned.",

        f"Engine confidence: {confidence}/100. "
        "The score is a rule-based quality measure, "
        "not a historical win probability.",
    ]
    ...
    if (
        ...
        reasons.append(
            f"Selected option PoP from Upstox is "
            f"{best_plan['pop']:.1f}%."
        )
