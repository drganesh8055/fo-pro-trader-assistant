...
def scan_fno_trade_alerts(risk_profile, top_alerts=8):
    ...
    progress = st.progress(0, text="Stage 1: scanning NSE F&O option chains...")
    ...
    for idx, item in enumerate(universe, start=1):
        ...
        progress.progress(
            ...
            text=f"Stage 1: {idx}/{len(universe)} F&O stocks"
        )
    ...
    alerts = []
    progress = st.progress(0, text="Stage 2: validating strongest candidates...")
    ...
    for idx, candidate in enumerate(shortlist, start=1):
        ...
        progress.progress(
            idx / max(len(shortlist), 1),
            text=f"Stage 2: {idx}/{len(shortlist)} candidates"
        )
...
with st.sidebar:
    ...
    if scan_market and scanner_enabled:
        try:
            ...
            st.session_state["fno_alert_scan_info"] = (
                total, scanned, failed, stage2_count
            )
            st.session_state["fno_alert_scan_time"] = datetime.now(
