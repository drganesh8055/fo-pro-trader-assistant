...
if best_plan:
    st.markdown("### 📍 1. WHEN TO ENTER")
    st.markdown(
        f"""
        <div style="border:1px solid rgba(128,128,128,.25);border-radius:10px;padding:18px;margin-bottom:16px;background:rgba(128,128,128,.04);">
            <div style="font-size:15px;font-weight:600;margin-bottom:10px;">Wait for confirmation</div>
            <div style="font-size:15px;line-height:1.6;">{best_plan["trigger"]}</div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown('<div style="text-align:center;font-size:24px;margin:-6px 0 10px 0;">↓</div>', unsafe_allow_html=True)

    st.markdown("### 🟢 2. ENTRY")
    st.markdown(
        f"""
        <div style="border:1px solid rgba(128,128,128,.25);border-radius:10px;padding:18px;margin-bottom:16px;background:rgba(128,128,128,.04);text-align:center;">
            <div style="font-size:14px;margin-bottom:8px;opacity:.8;">Option Premium</div>
            <div style="font-size:26px;font-weight:700;">{fmt_price(best_plan["entry"])}</div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown('<div style="text-align:center;font-size:24px;margin:-6px 0 10px 0;">↓</div>', unsafe_allow_html=True)

    st.markdown("### 🔴 3. STOP LOSS")
    st.markdown(
        f"""
        <div style="border:1px solid rgba(128,128,128,.25);border-radius:10px;padding:18px;margin-bottom:16px;background:rgba(128,128,128,.04);text-align:center;">
            <div style="font-size:26px;font-weight:700;">{fmt_price(best_plan["sl"])}</div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown('<div style="text-align:center;font-size:24px;margin:-6px 0 10px 0;">↓</div>', unsafe_allow_html=True)

    st.markdown("### 🎯 4. PROFIT TARGETS")
    st.markdown(
        f"""
        <div style="border:1px solid rgba(128,128,128,.25);border-radius:10px;overflow:hidden;margin-bottom:16px;">
            <div style="display:flex;padding:14px 16px;border-bottom:1px solid rgba(128,128,128,.20);">
                <div style="flex:1;font-weight:600;">Target 1</div>
                <div style="font-weight:700;">{fmt_price(best_plan["target1"])}</div>
            </div>
            <div style="display:flex;padding:14px 16px;">
                <div style="flex:1;font-weight:600;">Target 2</div>
                <div style="font-weight:700;">{fmt_price(best_plan["target2"])}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("### 🚪 5. WHEN TO EXIT")
    st.markdown(
        f"""
        <div style="border:1px solid rgba(128,128,128,.25);border-radius:10px;padding:18px;margin-bottom:20px;background:rgba(128,128,128,.04);">
            <div style="font-size:15px;font-weight:600;margin-bottom:10px;">Exit if:</div>
            <div style="font-size:15px;line-height:1.7;">• {best_plan["exit"]}</div>
            <div style="margin-top:12px;font-size:15px;font-weight:600;">After Target 1 → Book partial profit + trail remaining position</div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown('<div style="border-top:1px solid rgba(128,128,128,.25);margin-top:8px;padding-top:18px;"><div style="font-size:18px;font-weight:700;margin-bottom:18px;">📊 Trade Quality</div></div>', unsafe_allow_html=True)
    pop_value = f"{best_plan['pop']:.1f}%" if np.isfinite(best_plan["pop"]) else "Not returned"
    q1, q2 = st.columns(2)
    with q1:
        st.markdown("**Probability of Profit**")
        st.markdown(f'<div style="font-size:28px;font-weight:700;margin-top:4px;margin-bottom:14px;">{pop_value}</div>', unsafe_allow_html=True)
    with q2:
        st.markdown("**Risk / Reward**")
        st.markdown(f'<div style="font-size:15px;line-height:1.8;margin-top:4px;">Target 1 → <b>1:{best_plan["rr1"]:.2f}</b><br>Target 2 → <b>1:{best_plan["rr2"]:.2f}</b></div>', unsafe_allow_html=True)

st.caption(
    "ℹ️ PoP = Probability of Profit provided by Upstox for the option contract; "
    "it is an estimate, not a guarantee of profit. Entry, SL and targets are engine-derived levels, not guaranteed executions."
)
st.markdown('</div>', unsafe_allow_html=True)
