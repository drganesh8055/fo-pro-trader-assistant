...
/* ============================================================
   F&O SCANNER — LEFT SIDEBAR ONLY
   ============================================================ */
.scanner-title {font-size:16px;font-weight:900;color:#182230;margin:2px 0 8px;}
.scanner-alert {border-radius:12px;padding:10px 11px;margin:7px 0;border:1px solid #dfe3e8;background:#fff;box-shadow:0 2px 7px rgba(16,42,67,.05);}
.scanner-green {background:linear-gradient(180deg,#ecfdf3,#ffffff);border-color:#86efac;}
.scanner-red {background:linear-gradient(180deg,#fff1f2,#ffffff);border-color:#fda4af;}
.scanner-rank {font-size:12px;font-weight:900;color:#344054;}
.scanner-action {font-size:15px;font-weight:950;margin-top:3px;color:#182230;}
.scanner-green .scanner-action {color:#147a3d;}
.scanner-red .scanner-action {color:#b4232f;}
.scanner-contract {font-size:14px;font-weight:850;color:#475467;margin-top:2px;}
.scanner-stats {display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:7px;}
.scanner-stats span {background:rgba(255,255,255,.75);border:1px solid rgba(152,162,179,.22);border-radius:7px;padding:5px 6px;font-size:10px;color:#667085;}
.scanner-stats b {color:#182230;font-size:12px;}
.scanner-meta {font-size:9px;color:#667085;margin-top:7px;}
...
with st.sidebar:
    ...
    # ------------------------------------------------------------
    # BACKEND F&O SCANNER
    # This adds alerts only to the left sidebar. The main dashboard
    # on the right is intentionally untouched.
    # ------------------------------------------------------------
    scanner_enabled = st.checkbox("🔔 F&O Scanner", value=st.session_state.get("scanner_enabled", False))
    st.session_state["scanner_enabled"] = scanner_enabled
    scanner_slots = st.selectbox("Scanner depth", [8, 12, 16], index=1, help="Number of strongest F&O candidates sent through the full engine after the market-wide fast filter.")
    if scanner_enabled:
        st.caption("Scans NSE F&O stocks in the backend and shows only setups that pass the same 5-way engine gates.")
    st.divider()
...
# ============================================================
# MARKET-WIDE F&O SCANNER
# ============================================================
# The scanner uses a two-stage design so it can cover the NSE F&O universe
# without hammering the Upstox REST API. Stage 1 gets live quotes for the
# entire F&O-stock universe in batches. Stage 2 runs the SAME option-chain,
# technical, OI, Greeks, liquidity, PoP and quality gates used by the main
# analyzer on only the strongest live candidates.
#
# Upstox documents the NSE instrument JSON as the complete BOD instrument
# universe and the market-quote endpoint supports up to 500 instruments per
# request. The scanner therefore avoids one-request-per-stock quote bursts.
# ============================================================

FNO_INSTRUMENT_URL = "https://assets.upstox.com/market-quote/instruments/exchange/NSE.json.gz"

@st.cache_data(ttl=21600, show_spinner=False)
def get_nse_fno_stock_universe():
    response = requests.get(FNO_INSTRUMENT_URL, timeout=30)
    response.raise_for_status()
    raw = gzip.decompress(response.content)
    payload = json.loads(raw.decode("utf-8"))
    if not isinstance(payload, list):
        raise UpstoxError("Upstox NSE instrument file returned an unexpected format.")

    # Use current NSE stock futures to identify stocks that actually have an
    # F&O underlying. One underlying key is retained per stock.
    today = date.today()
    universe = {}
    for item in payload:
        if str(item.get("segment", "")) != "NSE_FO":
            continue
        if str(item.get("instrument_type", "")).upper() != "FUT":
            continue
        if str(item.get("underlying_type", "")).upper() != "EQUITY":
            continue
        underlying_key = str(item.get("underlying_key", "")).strip()
        underlying_symbol = alias_symbol(str(item.get("underlying_symbol", "")))
        if not underlying_key or not underlying_symbol:
            continue
        expiry_raw = item.get("expiry")
        expiry_date = None
        try:
            # Instrument BOD files may expose expiry as epoch milliseconds.
            if isinstance(expiry_raw, (int, float)):
                expiry_date = datetime.fromtimestamp(float(expiry_raw) / 1000, tz=ZoneInfo("Asia/Kolkata")).date()
            else:
        ...
        if current is None or (expiry_date and expiry_date < current["expiry"]):
            universe[underlying_key] = {
                "symbol": underlying_symbol,
                "underlying_key": underlying_key,
                "expiry": expiry_date or today,
            }
...
def get_quotes_batch(instrument_keys):
    ...
    for start in range(0, len(keys), 500):
        ...
        for returned_key, quote_data_item in (payload.get("data") or {}).items():
            results[returned_key.replace(":", "|", 1)] = quote_data_item
    return results


def scanner_candidate_plan(symbol, underlying_key, risk_profile):
    try:
        contracts = get_contracts(underlying_key)
        expiries = available_expiries(contracts)
        if not expiries:
        ...
        chain = normalize_chain(get_option_chain(underlying_key, expiry))
        ...
        candles_30m = get_30m_candles(underlying_key)
        candles_5m = get_intraday_candles(underlying_key, 5)
        daily = technicals(candles, spot)
        tf30_local = technicals(candles_30m, spot)
        tf5_local = technicals(candles_5m, spot)
        support_local, resistance_local, pcr_local, oi_info_local = oi_levels(chain, spot)
        regime_local = market_regime(tf5_local, tf30_local, daily, spot)
        atm_idx = (chain["Strike"] - spot).abs().idxmin()
        local_band = chain.iloc[max(0, atm_idx - 8):min(len(chain), atm_idx + 9)].copy()

        def local_buy(side):
            candidates = []
            prefix = "CE" if side == "CE" else "PE"
            for _, row in local_band.iterrows():
                scored = score_option(row, side, spot, pcr_local, tf5_local, tf30_local, daily, chain,
                                      support=support_local, resistance=resistance_local,
                                      oi_wall_info=oi_info_local, regime=regime_local)
                ...
                if np.isfinite(pop) and np.isfinite(ltp) and ltp > 0:
                    candidates.append((scored["score"], pop, volume, -spread, -abs(delta - .55), row))
            if not candidates:
                return None
            row = max(candidates, key=lambda x: x[:-1])[-1]
            return build_plan(row, side, spot, support_local, resistance_local, pcr_local,
                              tf5_local, tf30_local, daily, risk_profile, chain, oi_info_local, regime_local)
        ...
        def local_sell(side):
            ...
            for _, row in chain.iterrows():
                ...
                if side == "PE" and strike > spot * 0.995:
                    continue
                plan = build_sell_plan(row, side, spot, support_local, resistance_local, pcr_local,
                                       tf5_local, tf30_local, daily, risk_profile, chain, oi_info_local, regime_local)
                if plan is not None:
                    candidates.append((plan["score"], plan["pop"], plan["volume"], -plan["spread_pct"],
                                       -abs(abs(plan["delta"]) - .30), plan))
            return max(candidates, key=lambda x: x[:-1])[-1] if candidates else None

        plans = {
            "CALL BUY": local_buy("CE"),
            "CALL SELL": local_sell("CE"),
            "PUT BUY": local_buy("PE"),
            "PUT SELL": local_sell("PE"),
        }
        eligible = []
        for action, plan in plans.items():
            ...
            if action in {"CALL BUY", "PUT BUY"}:
                ready = (score >= 65 and pop >= 55 and alignment >= 2 and not hard_fail
                         and plan.get("readiness") == "READY"
                         and plan.get("candle_confirmed") and plan.get("volume_confirmed"))
            else:
                ready = (score >= 65 and pop >= 55 and alignment >= 2 and not hard_fail
                         and plan.get("readiness") == "READY" and safe_float(plan.get("rr1"), 0) >= 0.75)
            if ready:
                eligible.append((score + pop * 0.20 + min(safe_float(plan.get("volume"), 0) / 100000, 3), action, plan))

        if not eligible:
            return None
        _, action, plan = max(eligible, key=lambda x: x[0])
        return {
            "symbol": symbol, "action": action, "strike": safe_float(plan.get("strike")),
            "side": plan.get("side"), "pop": safe_float(plan.get("pop")),
            "score": safe_float(plan.get("score"), 0), "entry": safe_float(plan.get("entry")),
            "sl": safe_float(plan.get("sl")), "target1": safe_float(plan.get("target1")),
            "target2": safe_float(plan.get("target2")), "spot": spot, "expiry": expiry,
            "lot_size": next((int(round(safe_float(c.get("lot_size")))) for c in expiry_contracts
                               if np.isfinite(safe_float(c.get("lot_size"))) and safe_float(c.get("lot_size")) > 0), None),
            "regime": regime_local, "plan": plan,
        }
    except (UpstoxRateLimitError, UpstoxError):
        raise
    except Exception:
        return None
...
def scan_fno_market(risk_profile, scan_depth, exclude_symbol):
    universe = get_nse_fno_stock_universe()
    universe = [x for x in universe if x["symbol"] != alias_symbol(exclude_symbol)]
    if not universe:
        return []

    # Stage 1: quote every F&O stock in batches.
    quotes = get_quotes_batch([x["underlying_key"] for x in universe])
    fast = []
    for item in universe:
        q = quotes.get(item["underlying_key"]) or quotes.get(item["underlying_key"].replace("|", ":", 1))
        if not q:
            continue
        ltp = safe_float(q.get("last_price"))
        prev = safe_float(q.get("prev_close_price"), ltp)
        volume = safe_float((q.get("ohlc") or {}).get("volume"), 0)
        if not np.isfinite(ltp) or ltp <= 0 or not np.isfinite(prev) or prev <= 0:
            continue
        change_pct = (ltp - prev) / prev * 100
        oi = safe_float(q.get("oi"), 0)
        prev_oi = safe_float(q.get("previous_oi"), 0)
        oi_change = abs(oi - prev_oi) if np.isfinite(oi) and np.isfinite(prev_oi) else 0
        # Fast-stage ranking only decides which stocks receive the full engine.
        fast_score = abs(change_pct) * 4 + np.log1p(max(volume, 0)) * 1.2 + np.log1p(max(oi_change, 0)) * 0.8
        fast.append((fast_score, item, ltp, change_pct, volume))
    fast.sort(key=lambda x: x[0], reverse=True)

    results = []
    for _, item, _, _, _ in fast[:int(scan_depth)]:
        result = scanner_candidate_plan(item["symbol"], item["underlying_key"], risk_profile)
        if result:
            results.append(result)
    results.sort(key=lambda x: (x["score"], x["pop"]), reverse=True)
    return results[:8]
...
# ------------------------------------------------------------
# LEFT-SIDE F&O ALERTS
# Rendered after the main engine is calculated so the scanner can
# reuse the exact same backend functions without changing the right UI.
# ------------------------------------------------------------
if st.session_state.get("scanner_enabled", False):
    try:
        with st.sidebar:
            st.markdown("<div class='scanner-title'>🔔 F&O TRADE ALERTS</div>", unsafe_allow_html=True)
            now_scan = datetime.now(ZoneInfo("Asia/Kolkata"))
            market_open_scan = now_scan.weekday() < 5 and (now_scan.hour, now_scan.minute) >= (9, 15) and (now_scan.hour, now_scan.minute) <= (15, 30)
            if not market_open_scan:
                st.info("Scanner is armed. Alerts will populate during NSE market hours.")
            else:
                with st.spinner("Scanning F&O stocks..."):
                    scanner_results = scan_fno_market(risk_profile, scanner_slots, symbol)
                if scanner_results:
                    st.caption(f"{len(scanner_results)} actionable setup(s) · same engine gates")
                    for idx, alert in enumerate(scanner_results, 1):
                        action = alert["action"]
                        tone = "green" if action in {"CALL BUY", "PUT SELL"} else "red"
                        contract = f"{alert['strike']:.0f} {'CE' if alert['side']=='CE' else 'PE'}"
                        st.markdown(
                            f"""<div class='scanner-alert scanner-{tone}'>
                            <div class='scanner-rank'>#{idx} · {alert['symbol']}</div>
                            <div class='scanner-action'>{action}</div>
                            <div class='scanner-contract'>{contract}</div>
                            <div class='scanner-stats'><span>PoP <b>{alert['pop']:.1f}%</b></span><span>Q <b>{alert['score']:.0f}/100</b></span></div>
                            <div class='scanner-meta'>Entry {fmt_price(alert['entry'])} · SL {fmt_price(alert['sl'])}</div>
                            </div>""", unsafe_allow_html=True)
                        if st.button(f"Analyze {alert['symbol']}", key=f"scanner_open_{alert['symbol']}", use_container_width=True):
                            st.session_state["symbol"] = alert["symbol"]
                            st.rerun()
                else:
                    st.warning("No F&O stock currently passed the full engine gates.")
    except UpstoxRateLimitError as exc:
        with st.sidebar:
            st.warning(f"Scanner paused: Upstox rate limit. Wait about {exc.retry_after}s.")
    except Exception as scanner_exc:
        with st.sidebar:
            st.warning(f"Scanner unavailable: {scanner_exc}")
