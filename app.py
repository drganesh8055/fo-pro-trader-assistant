...
.scanner-alert { display:flex; gap:10px; align-items:flex-start; padding:10px; margin:7px 0 4px; border-radius:10px; border:1px solid #dfe4ea; background:#fff; }
.scanner-call { border-left:4px solid #16a34a; background:#f0fdf4; }
.scanner-put { border-left:4px solid #dc2626; background:#fff1f2; }
.scanner-rank { font-size:12px; font-weight:900; min-width:18px; }
.scanner-main { display:flex; flex-direction:column; gap:2px; line-height:1.2; }
.scanner-main b { font-size:14px; }
.scanner-main span { font-size:11px; font-weight:800; }
.scanner-main small { font-size:10px; color:#475467; }
...
# ============================================================
# FULL F&O MARKET SCANNER — LEFT SIDEBAR ONLY
# Uses the SAME option-quality + build_plan + decision gates used by the
# selected-instrument analyzer. The right-side dashboard is not modified.
# ============================================================
...
def get_fno_underlyings():
    """Build the current NSE equity F&O universe from Upstox BOD master."""
    ...
    for item in records:
        ...
        symbol_name = str(item.get("underlying_symbol") or "").strip().upper()
        if not key or not symbol_name:
            continue
        current = universe.get(key)
        if current is None or expiry_date < current["expiry"]:
            universe[key] = {"symbol": symbol_name, "underlying_key": key, "expiry": expiry_date}
...
def _scanner_stage1_candidates(rows, spot_value):
    """Fast option-chain filter before expensive technical confirmation."""
    candidates = []
    if not rows or not np.isfinite(spot_value) or spot_value <= 0:
        return candidates
    for raw in rows:
        strike = safe_float(raw.get("strike_price"))
        if not np.isfinite(strike):
            continue
        distance = abs(strike - spot_value) / spot_value * 100
        if distance > 5.0:
            continue
        for side in ("CE", "PE"):
            ...
            if not np.isfinite(pop) or pop < 55 or not np.isfinite(ltp) or ltp <= 0 or oi <= 0:
                continue
            spread_pct = ((ask - bid) / ltp * 100) if np.isfinite(ask) and np.isfinite(bid) and ltp > 0 and ask >= bid else 999
            candidates.append({"side": side, "strike": strike, "pop": pop, "volume": volume, "oi": oi, "spread_pct": spread_pct, "distance": distance})
    candidates.sort(key=lambda x: (-x["pop"], -x["volume"], x["spread_pct"], x["distance"]))
    return candidates[:6]

def scan_fno_same_engine(risk_profile, top_alerts=8):
    """Two-stage full-market scan.

    Stage 1 touches the full F&O equity universe using live option chains.
    Stage 2 runs the existing technicals/oi_levels/build_plan/_actionable logic
    only on the strongest candidates, reducing API load while preserving the
    same decision gates used by the main analyzer.
    """
    universe = get_fno_underlyings()
    stage1 = []
    failed = 0
    progress = st.progress(0, text=f"Scanning full NSE F&O universe: 0/{len(universe)}")

    for idx, item in enumerate(universe, 1):
        try:
            rows = get_scanner_chain(item["underlying_key"], item["expiry"])
            spot_values = [safe_float(r.get("underlying_spot_price")) for r in rows]
            spot_value = next((x for x in spot_values if np.isfinite(x) and x > 0), np.nan)
            quick = _scanner_stage1_candidates(rows, spot_value)
            if quick:
                stage1.append({**item, "rows": rows, "spot": spot_value, "quick": quick})
        except UpstoxRateLimitError:
            progress.empty()
            raise
        except Exception:
            failed += 1
        progress.progress(idx / max(len(universe), 1), text=f"Scanning full NSE F&O universe: {idx}/{len(universe)}")
    progress.empty()

    # Keep only the strongest chain candidates for expensive 5m/30m/daily work.
    stage1.sort(key=lambda x: (-x["quick"][0]["pop"], -x["quick"][0]["volume"], x["quick"][0]["spread_pct"]))
    technical_pool = stage1[:max(12, top_alerts * 2)]
    alerts = []

    progress2 = st.progress(0, text=f"Applying full engine to top {len(technical_pool)} candidates...")
    for idx, item in enumerate(technical_pool, 1):
        try:
            chain = normalize_chain(item["rows"])
            spot = item["spot"]
            ...
            # Technical data is identical to the main analyzer.
            candles = get_daily_candles(item["underlying_key"])
            candles_30m = get_30m_candles(item["underlying_key"])
            candles_5m = get_intraday_candles(item["underlying_key"], 5)
            ...
            for side in ("CE", "PE"):
                ...
                for _, row in candidate_rows.iterrows():
                    scored = score_option(row, side, spot, pcr, tf5, tf30, daily, chain, support=support, resistance=resistance, oi_wall_info=oi_wall_info)
                    side_rows.append((scored, row))
                if not side_rows:
                    continue
                best_scored, best_row = max(side_rows, key=_candidate_key)
                plan = build_plan(best_row, side, spot, support, resistance, pcr, tf5, tf30, daily, risk_profile, chain, oi_wall_info)
                if plan:
                    plans.append(plan)

            ce_plan = next((p for p in plans if p["side"] == "CE"), None)
            pe_plan = next((p for p in plans if p["side"] == "PE"), None)
            ce_score = ce_plan["score"] if ce_plan else 0
            pe_score = pe_plan["score"] if pe_plan else 0
            overall_direction, overall_alignment = overall_trend(tf5, tf30, daily)

            ce_actionable = bool(ce_plan and ce_score >= 65 and ce_plan.get("alignment", 0) >= 2 and overall_direction == "Bullish" and not ce_plan.get("fail_reasons") and ce_plan.get("pop_ok") and ce_plan.get("readiness") == "READY")
            pe_actionable = bool(pe_plan and pe_score >= 65 and pe_plan.get("alignment", 0) >= 2 and overall_direction == "Bearish" and not pe_plan.get("fail_reasons") and pe_plan.get("pop_ok") and pe_plan.get("readiness") == "READY")

            if ce_actionable and (not pe_actionable or ce_score >= pe_score + 5):
                plan, action = ce_plan, "CALL BUY"
            elif pe_actionable and (not ce_actionable or pe_score >= ce_score + 5):
                plan, action = pe_plan, "PUT BUY"
            else:
                plan, action = None, "NO TRADE"
                valid_wait = []
                for p, s, side in ((ce_plan, ce_score, "CE"), (pe_plan, pe_score, "PE")):
                    if p and s >= 58 and not p.get("fail_reasons") and p.get("alignment", 0) >= 1 and tf5.get("trend") != ("Bearish" if side == "CE" else "Bullish") and (overall_direction in {"Bullish" if side == "CE" else "Bearish", "Mixed"} or p.get("alignment", 0) >= 2) and p.get("pop_ok"):
                        valid_wait.append((s, p))
                if valid_wait:
                    _, plan = max(valid_wait, key=lambda x: x[0])
                    action = plan.get("readiness", "WATCHLIST") if plan.get("readiness") != "READY" else "WATCHLIST"

            # Alerts are deliberately restricted to actionable signals. This
            # prevents the left panel becoming a list of weak candidates.
            if action in {"CALL BUY", "PUT BUY"} and plan:
                alerts.append({
                    "symbol": item["symbol"], "action": action, "strike": plan["strike"], "pop": plan["pop"],
                    "score": plan["score"], "entry": plan["entry"], "sl": plan["sl"], "target1": plan["target1"],
                    "target2": plan["target2"], "expiry": item["expiry"], "underlying_key": item["underlying_key"],
                    "alignment": plan["alignment"], "spread": plan["spread_pct"], "readiness": plan["readiness"],
                })
        except UpstoxRateLimitError:
            progress2.empty()
            raise
            ...
            pass
        progress2.progress(idx / max(len(technical_pool), 1), text=f"Applying full engine: {idx}/{len(technical_pool)}")
    progress2.empty()

    alerts.sort(key=lambda x: (-x["score"], -x["pop"], -x["alignment"]))
    return alerts[:top_alerts], len(universe), len(stage1), failed

# ============================================================
# SIDEBAR — SCANNER IS LEFT-SIDE ONLY
# ============================================================
with st.sidebar:
    st.markdown("### 🔔 F&O TRADE ALERTS")
    st.caption("Full NSE equity F&O scan using the same engine gates as the analyzer.")

    scanner_enabled = st.toggle("Scanner ON / OFF", value=st.session_state.get("fno_scanner_enabled", False), key="fno_scanner_enabled")
    alert_count = st.selectbox("Maximum alerts", [5, 8, 12], index=1, key="fno_alert_count")

    scan_now = st.button("🔎 Scan Full F&O Market", use_container_width=True, type="primary", disabled=not scanner_enabled)

    if scanner_enabled and scan_now:
        try:
            with st.spinner("Scanning F&O market with the same decision engine..."):
                results, total, stage1_count, failed = scan_fno_same_engine(st.session_state.get("risk_profile", "Balanced"), alert_count)
            st.session_state["fno_engine_alerts"] = results
            st.session_state["fno_engine_scan_info"] = (total, stage1_count, failed)
            st.session_state["fno_engine_scan_time"] = datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%H:%M:%S")
        except UpstoxRateLimitError as exc:
            st.session_state["fno_engine_alerts"] = []
            st.error(f"⏳ Upstox rate limit reached. Wait about {exc.retry_after} seconds before scanning again.")
        except Exception as exc:
            st.session_state["fno_engine_alerts"] = []
            st.error(f"F&O scanner failed: {exc}")

    alerts = st.session_state.get("fno_engine_alerts", [])
    info = st.session_state.get("fno_engine_scan_info")
    scan_time = st.session_state.get("fno_engine_scan_time")

    if not scanner_enabled:
        st.info("Scanner is OFF. Your right-side analyzer works normally.")
    elif alerts:
        st.success(f"{len(alerts)} actionable setup(s) found")
        if info:
            total, stage1_count, failed = info
            st.caption(f"Universe: {total} • Chain candidates: {stage1_count} • Unavailable: {failed}")
        if scan_time:
            st.caption(f"Last scan: {scan_time} IST")

        for idx, alert in enumerate(alerts, 1):
            action_cls = "scanner-call" if alert["action"] == "CALL BUY" else "scanner-put"
            side_icon = "🟢" if alert["action"] == "CALL BUY" else "🔴"
            st.markdown(
                f"""<div class='scanner-alert {action_cls}'>
                    <div class='scanner-rank'>{idx}</div>
                    <div class='scanner-main'>
                        <b>{side_icon} {alert['symbol']}</b>
                        <span>{alert['action']} • {alert['strike']:.0f} {'CE' if alert['action']=='CALL BUY' else 'PE'}</span>
                        <small>PoP {alert['pop']:.1f}% • Quality {alert['score']:.0f}/100</small>
                        <small>Entry {fmt_price(alert['entry'])} • SL {fmt_price(alert['sl'])} • T1 {fmt_price(alert['target1'])}</small>
                    </div>
                </div>""",
                unsafe_allow_html=True,
            )
            if st.button(f"Analyze {alert['symbol']}", key=f"scanner_analyze_{idx}_{alert['symbol']}", use_container_width=True):
                st.session_state["symbol"] = alert["symbol"]
                st.rerun()
    elif "fno_engine_alerts" in st.session_state:
        st.warning("No CALL BUY / PUT BUY setup currently passes the same engine gates.")
    ...
    symbol_input = st.text_input(symbol_label, value=symbol_default, placeholder=symbol_placeholder, label_visibility="collapsed")

    risk_profile = st.selectbox("Risk Profile", ["Conservative", "Balanced", "Aggressive"], index=1, key="risk_profile")
